...
function CPluginManager:LoadFromFile(file)
	self.Plugins = self.Plugins or {};
	local succ, pgin = pcall(require, file)
	if( not succ ) then
		print("Plugin Error: "..pgin);
	...
	if( type(pgin) == "table" ) then
		self.Plugins[pgin.Id] = pgin;
		succ, err = pcall(self.Plugins[pgin.Id].Init, self.Plugins[pgin.Id]);
		if( not succ ) then
			print("Plugin Error ["..self.Plugins[pgin.Id].Name.."]: "..err);
			print("Plugin Removed: "..self.Plugins[pgin.Id].Name);
			self.Plugins[pgin.Id] = nil;
		end
	end
end
